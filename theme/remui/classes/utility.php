...
class utility {
        ...
        foreach ($categoryTree as $category) {
            ...
            if ($hasChildren) {
                $html .= '<div class="menu-heading d-flex flex-gap-2 w-100">
                            ...
                            <a href="'.$CFG->wwwroot.'/course/index.php?categoryid='.$category->id.'" data-cat-id="'.$category->id.'" class="category-link ellipsis">'.format_text($category->name, FORMAT_HTML).'</a>
                        </div>';
                $html .= '<div id="collapse' .$uniquenumber. '" class="collapse">
                            <div class="menu-body">';
            } else {
                $html .= '<ul class="m-0 p-pl-5">
                                <li><a href="'.$CFG->wwwroot.'/course/index.php?categoryid='.$category->id.'" data-cat-id="'.$category->id.'" class="category-link ellipsis">'.format_text($category->name, FORMAT_HTML).'</a></li>
                            </ul>';
            }
    ...
    public static function get_homepage_depriation_modal() {
        global $OUTPUT, $CFG;
        $homepagedeptemplate = [
            'dephomepagerightimgurl' => \theme_remui\toolbox::image_url('homepagedepmodalrightisideimg', 'theme_remui'),
            'wwwroot' => $CFG->wwwroot,
        ];
        return $OUTPUT->render_from_template("theme_remui/homepage_deprication_modal", $homepagedeptemplate);
